...
def generateIndex(report_dir, reportlist, f, runpath, default_order):
    # Init user info
    userinfo = UserInfo()
    userinfo.realdir =  \
    ...
    addReportTable(f, buglist, default_order)
...
def addReportTable(f, buglist, default_order):
    f.write('''
<h2>Reports</h2>
...
def generate(reports, report_names):
    default_order = False
    with open(reports + os.sep + 'index.html', 'w') as index:
        if not report_names:
            default_order = True
            report_names = os.listdir(reports)
        generateIndex(reports, report_names, index, __file__, default_order)
